...
function SingleCard({ title, link, thumbnail, watched }: { title: string, link: string, thumbnail: string, watched: boolean }) {
    ...
    return (
        <div className="  group md:ease-out duration-500  hover:-translate-y-12    h-[15vh]">


            <div

                className="  relative brightness-75   font-thin text-xl text-white   md:hover:text-2xl hover:brightness-100  bg-cover bg-opacity-50 h-[13vh] md:h-[15vh] shadow-xl m-3 rounded-xl flex justify-start  items-end text-center" style={{ backgroundImage: `url(${thumbnail})`, }}  >
            ...
            <div id="remove" className="  w-full flex justify-end px-4">

                <div onClick={() => { remover(link) }} className=" hidden group-hover:block hover:bg-red-700 bg-red-900 rounded-lg text-[10px]    inline-block p-1"> remove</div>
            </div>
